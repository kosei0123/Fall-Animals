...
public class RockMove_teppen : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        {
            case "Rock0":
                rbRock.velocity = new Vector3(-3.0f - ((float)PlayerPrefs.GetInt("TeppenFloor") / 2.0f) + TeppenShopUI.RockSlowRealTotal, randomRockVelocity_Y, 0);
                break;
            case "Rock1":
                rbRock.velocity = new Vector3(-6.0f - ((float)PlayerPrefs.GetInt("TeppenFloor") / 2.0f) + TeppenShopUI.RockSlowRealTotal, randomRockVelocity_Y, 0);
                break;
            case "Rock2":
                rbRock.velocity = new Vector3(-9.0f - ((float)PlayerPrefs.GetInt("TeppenFloor") / 2.0f) + TeppenShopUI.RockSlowRealTotal, randomRockVelocity_Y, 0);
                break;
            case "Rock3":
                rbRock.velocity = new Vector3(3.0f + ((float)PlayerPrefs.GetInt("TeppenFloor") / 2.0f) - TeppenShopUI.RockSlowRealTotal, randomRockVelocity_Y, 0);
                break;
