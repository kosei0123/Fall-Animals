...
public class Stage4Move_offline : MonoBehaviour
{
    ...
    void Update()
    {
        //-10より小さければ上に上がり、5より大きければ下に下がる
        if (Stage4.transform.position.y < -10.0f) moveUpFlag = true;
        else if (Stage4.transform.position.y > 5.0f) moveUpFlag = false;

        switch (moveUpFlag)
        {
            case true:
                //真ん中
                Stage4.transform.position = new Vector3(Stage4.transform.position.x, Stage4.transform.position.y + 0.04f, Stage4.transform.position.z);
                //横キューブ
                Stage4Cube.transform.position = new Vector3(Stage4Cube.transform.position.x, Stage4Cube.transform.position.y + 0.02f, Stage4Cube.transform.position.z);
                Stage4Cube.transform.localScale = new Vector3(Stage4Cube.transform.localScale.x, Stage4Cube.transform.localScale.y + 0.02f, Stage4Cube.transform.localScale.z);
                Stage4Cube2.transform.position = new Vector3(Stage4Cube2.transform.position.x, Stage4Cube2.transform.position.y + 0.02f, Stage4Cube2.transform.position.z);
                Stage4Cube2.transform.localScale = new Vector3(Stage4Cube2.transform.localScale.x, Stage4Cube2.transform.localScale.y + 0.02f, Stage4Cube2.transform.localScale.z);
                break;
            case false:
                //真ん中
